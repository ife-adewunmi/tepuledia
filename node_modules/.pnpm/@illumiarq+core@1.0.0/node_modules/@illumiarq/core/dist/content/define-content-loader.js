...
export function defineContentLoader(options) {
    const { schema, extensions = [...CONTENT_DEFAULTS.extensions], highlight = CONTENT_DEFAULTS.highlight, } = options;
    ...
    return {
        ...
        async section(name) {
            ...
            return pages.filter((p) => p.path.startsWith(`${name}/`) || p.path.startsWith(`${name}${sep}`));
        },
        clearCache() {
...
function parsePage(filePath, absDir, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
schema, md) {
    ...
    const readingTime = Math.max(CONTENT_DEFAULTS.minReadingTimeMinutes, Math.ceil(countWords(body) / CONTENT_DEFAULTS.readingSpeedWpm));
