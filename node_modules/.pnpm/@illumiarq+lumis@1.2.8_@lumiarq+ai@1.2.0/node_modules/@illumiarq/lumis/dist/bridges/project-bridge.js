...
async function listRoutes(projectRoot) {
...
async function showConfig(projectRoot, configName) {
    const candidates = [
...
async function listScheduledJobs(projectRoot) {
...
async function runScheduledJob(projectRoot, jobName) {
    const providersFile = [
        join(projectRoot, 'bootstrap', 'providers.ts'),
        join(projectRoot, 'bootstrap', 'providers.js'),
    ].find((f) => existsSync(f));
    ...
    if (typeof scheduler.runByName === 'function') {
        await scheduler.runByName(jobName);
    }
    else {
