...
async function runListenerWithTimeout(entry, base) {
    ...
    try {
        await Promise.race([
            ...
            new Promise((_, reject) => controller.signal.addEventListener('abort', () => reject(new ListenerTimeoutError(base.name, entry.timeoutMs)))),
        ]);
    }
    catch (err) {
        ...
        for (const entry of entries) {
            try {
                ...
                if (result instanceof Promise) {
                    result.catch((err) => console.error(`[EventBus] Listener error for "${event.name}":`, err));
                }
            }
            catch (err) {
