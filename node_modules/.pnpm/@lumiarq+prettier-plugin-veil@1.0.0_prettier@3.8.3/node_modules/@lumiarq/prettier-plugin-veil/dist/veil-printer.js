...
function printDirective(node, depth, opts) {
    return `${indent(depth, opts)}${formatDirectiveLine(node.name, node.args)}`;
}
function printBlock(node, depth, opts) {
    const parts = [];
...
function printRaw(node, depth, opts) {
...
export function printVeil(nodes, options = {}) {
    ...
    const sections = [];
    if (hoisted.length > 0)
        sections.push(hoisted.join('\n'));
    if (rest.length > 0)
        sections.push(rest.join('\n'));
...
//# sourceMappingURL=veil-printer.js.map