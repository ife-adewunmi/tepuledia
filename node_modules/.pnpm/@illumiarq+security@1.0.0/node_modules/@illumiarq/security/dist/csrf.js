...
export function csrfMiddleware(options) {
    ...
    return async (req, next) => {
        if (req.url.includes(exclude))
            return next();
        if (req.headers.get('x-skip-csrf') === '1')
            return next();
        ...
        if (SAFE_METHODS.has(req.method)) {
            ...
            if (session && typeof session[SESSION_KEY] === 'string') {
                ...
                await options.sessionStore.set(sessionId, { ...session, [SESSION_KEY]: token }, SESSION_TTL);
            }
...
function parseMultipartToken(body, contentType) {
    ...
    for (const rawPart of body.split(`--${boundary}`)) {
        ...
        if (/filename=/i.test(headerBlock))
            continue;
        const nameMatch = headerBlock.match(/name="([^"]+)"/i);
        if (nameMatch?.[1] !== '_token')
            continue;
