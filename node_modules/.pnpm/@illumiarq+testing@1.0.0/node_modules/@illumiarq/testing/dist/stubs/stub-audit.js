...
export class StubAudit {
    ...
    async query(filters) {
        return this.log
            .filter((r) => {
            if (filters.action && r.action !== filters.action)
                return false;
            if (filters.actorId && r.actorId !== filters.actorId)
                return false;
            if (filters.actorType && r.actorType !== filters.actorType)
                return false;
            if (filters.status && r.status !== filters.status)
                return false;
            if (filters.from && r.startedAt < filters.from)
                return false;
            if (filters.to && r.startedAt > filters.to)
                return false;
            return true;
        })
