...
export function generateSitemap(options) {
    const { baseUrl, routes, sitemapEntries = [], lastmod = new Date().toISOString().slice(0, 10), } = options;
    const staticUrls = routes
        .filter((r) => r.strategy === 'ssg')
        .map((r) => urlToXml(baseUrl, r.route.path, lastmod));
    const dynamicUrls = sitemapEntries.map((e) => urlToXml(baseUrl, e.url, e.lastmod ?? lastmod, e.priority));
