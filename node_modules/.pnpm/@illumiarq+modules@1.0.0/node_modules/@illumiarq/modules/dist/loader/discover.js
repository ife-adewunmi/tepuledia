...
export async function discoverModules(_scheduler, importFn) {
    ...
    for (const entry of manifest.modules) {
        try {
            ...
            throw new Error(`Failed to load module "${entry.name}" from ${entry.path}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return new Map([...modules.entries()].sort((left, right) => left[1].priority - right[1].priority || left[0].localeCompare(right[0])));
}
