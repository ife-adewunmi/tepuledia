...
export async function generateManifest(options = {}) {
    ...
    const manifestPath = resolve(options.manifestPath ?? join(cwd, 'storage/framework/cache/modules.manifest.json'));
    ...
    modules.sort((left, right) => left.priority - right.priority || left.name.localeCompare(right.name));
    const manifest = {
        version: '1.0.0',
