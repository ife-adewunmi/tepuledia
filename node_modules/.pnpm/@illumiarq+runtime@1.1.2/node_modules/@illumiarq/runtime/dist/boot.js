...
import { authMiddleware as _auth, csrfMiddleware as _csrf, throttleMiddleware as _throttle, trazeMiddleware as _traze, setTrazeLogger, } from '@illumiarq/http';
...
export async function boot(hooks) {
    ...
    for (const routeDef of routes) {
        ...
        const baseHandler = typeof routeDef.handler === 'function'
            ? routeDef.handler
        ...
        const middlewareFns = routeMiddleware
            ...
            const getPriority = (fn) => {
                const name = routeMiddleware.find((mw) => typeof mw === 'string' && getMiddleware(mw)?.handler === fn);
        ...
        if (middlewareFns.length > 0) {
            handler = async (ctx) => {
                ...
                return composeMiddleware(middlewareFns)(req, wrappedHandler);
            };
        }
        else {
            handler = baseHandler;
        ...
        const requestScopedHandler = async (honoCtx) => {
